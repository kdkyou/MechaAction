...
void Character::PostUpdate()
{
	...
	auto pos = m_correctionMat *m_correctionMat * CameraManager::Instance().GetCurrentCamera().lock()->GetCamera()->GetCameraMatrix();
	pos *= CameraManager::Instance().GetCurrentCamera().lock()->GetCamera()->GetProjMatrix();
	auto translation = pos.Translation();
...
void Character::LockOn()
{
	...
	for (auto& obj : SceneManager::Instance().GetEnemyList())
	{
		...
		if (obj->Intersects(sphereInfo, nullptr))
		{
			...
			if (SearchDetect(targetPos, camMat, 50) == true)
			{
