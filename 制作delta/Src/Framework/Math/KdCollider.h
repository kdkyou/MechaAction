...
private:
	...
	{
		DirectX::XMMATRIX rotMat = DirectX::XMMatrixRotationQuaternion(DirectX::XMLoadFloat4(&obb.Orientation));
		axes[0] = rotMat.r[0]; // X軸
		axes[1] = rotMat.r[1]; // Y軸
		axes[2] = rotMat.r[2]; // Z軸
	}

	// OBBを任意の軸に投影して、min/maxを返す
	void ProjectOBB(const DirectX::XMVECTOR& center, const DirectX::XMVECTOR axes[3], const DirectX::XMVECTOR& extents, const DirectX::XMVECTOR& axis, float& outMin, float& outMax)
	{
		float centerProj = DirectX::XMVectorGetX(DirectX::XMVector3Dot(center, axis));

		float radius = 0.0f;
		for (int i = 0; i < 3; ++i)
		{
			float axisProj = DirectX::XMVectorGetX(DirectX::XMVector3Dot(axes[i], axis));
			float extent = DirectX::XMVectorGetByIndex(extents, i);
			radius += std::abs(axisProj * extent);
		}
